...
There is exactly one open folder or none. Opening or closing a folder closes every tab. The kernel's working directory follows: the folder root when one is open, otherwise the active file's directory, otherwise home. The directory is recorded on the receive thread, with no queue in front of it, so a run or a debug session - answered on a lane of their own, and both reading that directory - cannot start in the directory the user has just left. Moving the *running* kernel is a separate matter and goes on the execute lane, in order with the user's own cells, as a silent internal call that never appears in the console.
