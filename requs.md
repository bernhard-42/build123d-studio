...
### 7. Adopting `ocp_viewer_core`

Last, and deliberately so, because its scope leaves this repository. What stood here until 2026-08-13 was a sixteen-item plan across eight projects, and it is **built**: `ocp-viewer-core` exists as its own project owning both halves - the Python on PyPI and the JavaScript on npm, one version string - the standalone left ocp_vscode as `ocp_viewer`, and ocp_vscode, ocp_viewer and Jupyter CadQuery have all adopted it. That plan is removed rather than kept as a checklist of ticked boxes; what it decided lives in the code, and the reasoning that outlived it is in `ocp-viewer-core/Design.md`.

**`ocp-viewer-core/Design.md` is the entry point and is tracked. Read it rather than this section** for how the shared half works, what each of the three adopted hosts had to write, and the four ways a measurement answer gets home. What follows is only what build123d Studio has to do, and the decisions that are ours.

The shared half, in five lines: a host implements **`Comms`** (the transport, and the only class it writes); **`Session`** caches the two reads one show makes and scopes them, with the call's keywords, to that one call; **`Config`** owns the defaults, the precedence over them and the enums; **`Viewer`** carries the whole show family as bound methods, so completion sees the full signature; `keys.py` is the key vocabulary in both languages. On the JavaScript side `options.js`, `render.js`, `apply.js`, `notify.js`, `states.js`, `logo.js` and `page.js` are the policy every viewer must agree on. **All four viewers behave the same** - his ruling, 2026-08-13: *"We need to have user support and maintenance in mind. Only when all viewers act the same it is doable."* So a default of ours that differs from the other three is a defect, not a preference.

#### What we have to write

**1. A `StudioComms(Comms)` for the show path, and nothing at all for the backend.** The backend half is already the shape the core settled on, independently: `sidecar/measure_process.py` computes and **returns**, `main.on_viewer_changes` delivers on the websocket the request came in on, with the `if response is not None` guard. `Comms` is a client transport - every method exists because the core initiates something - and the backend always answers someone who already reached in, so it needs no method and gets none.

| method | what StudioComms does |
| --- | --- |
| `send_data(data, timeit)` | `split_buffers` on the tessellation, then `send_model` over the model socket. Returns `None` - there is no handle |
| `send_backend(data, timeit)` | the id-to-shape mapping, to the measurement backend |
| `send_config(config, timeit)` | a `ui` config block to a live viewer. **No path for this today**; the frontend gains one and dispatches it to `applyConfig` |
| `send_command(data, timeit)` | screenshot and `set_relative_time` are the only two commands the core leaves here. **Screenshot has to come** - his ruling, and it is the worked example of feature parity: `createPage` already answers it, so what this host owes is the command and a place to write the file |
| `status()` | the viewer's live state - the reply hop below |
| `workspace_config()` | our stored viewer settings - the reply hop below |
| `encode_config(config)` | the default, `keys.to_javascript`: our receiver is three-cad-viewer, like both page hosts |
| `is_handle(obj)` / `begin` / `end` | the defaults. No handle, and we read nothing out of the keyword scope |

**The one protocol change on the kernel-to-sidecar hop is that the mapping stops riding inside the model frame.** Today `transport.send_model` writes token, header, mapping and payload as one message and `on_model` hands the mapping to `Measurements.load`. The core calls `send_data(t)` and then `comms.send_backend({"model": mapping})`, in that order, as two calls - so `modelsock`'s layout splits into a model message and a mapping message. Small, but it moves the framing, the sidecar's dispatch and both suites' fixtures together.

**2. The reply hop, and it is the one mechanism genuinely missing.** Kernel to sidecar is one-way: `transport.send_model` connects, sends and closes, and `modelsock` only reads. But `status()` and `workspace_config()` are *reads*, made by `_tessellate` before any model is sent.

Two halves, and only the second is new work in the transport:

- **The sidecar holds the picture, and never asks the browser.** No host asks its frontend anything synchronously - the VS Code extension answers a status request out of a copy the webview pushed earlier, and the core's `createNotifier` keeps exactly that running picture. Our frontend already sends `viewer.changes` deltas over the sidecar's websocket, because that is measurement's path; the sidecar accumulates the **state** keys from them and answers out of memory. `workspace_config()` is answered from `settings.json`, which the sidecar can read without asking anybody.
- **The model socket gains a request and one reply.** The kernel writes a command frame, waits for a single answer on the same connection, and closes it as it does today.

**Event keys are never accumulated, and that rule is ours generalised.** `notify.js`'s `EVENT_KEYS` - `selectedShapeIDs`, `selected`, `lastPick`, `activeTool`, `relative_time` - report that something *happened*; accumulating one replays a selection made minutes ago into the next measurement, against a model that may not contain those ids. `src/viewer/viewer.js` sends the delta for exactly that reason, the core now does it for every host, and the sidecar's accumulator must take the core's partition rather than a second opinion.

**Our status can be fresh, and the core must never rely on it.** Everywhere else the frontend reports back only when the kernel is idle, so a tree change made while a cell runs is invisible to a `show()` that same cell calls - "every show first gets the tree status and reapplies it" is a best effort, not a promise. Our reply hop has no kernel in the path. Keep that property; do not let anything shared depend on it, or the same script behaves differently here than in VS Code.

**3. `exclude_keys` - the keywords this surface may not be told.** Proposed, and it wants a ruling: `cad_width` and `height`, because the pane's layout decides them and the show signature must not fight the splitter; `viewer`, `anchor` and `pinning`, which name a notebook sidecar we do not have; and `port`, because there is nothing to dial - the model socket's port is OS-assigned and passed in the environment. Note that this list runs in the opposite direction to Jupyter CadQuery's, which excludes `port` alone and accepts the sizing keys, and that is the argument for it being per host at all.

**4. `workspace_config()` means the viewer parameters in the Settings dialog** - his ruling, 2026-08-13 - as ocp_vscode's means its `OcpCadViewer.view.*` section. Not the editor's own settings: the tabs, the shortcut map, the package sources, the splitter fractions and the workspace are ours and are not viewer config.

**Today that is exactly one key, `theme`**, and its tri-state maps onto the core's without a conversion: unpinned follows the desktop and is the core's `"browser"`, pinned is `"light"` or `"dark"`. Everything else the core asks about is filled from its own defaults, which is the same answer a first-run VS Code user gets.

**Later, and deliberately not part of the adoption: a "Viewer" tab in the Settings dialog** holding the viewer-related workspace settings the VS Code extension offers, with **"Dark mode" moving there** from the Editor tab's Appearance group where it sits today. Then `workspace_config()` answers from that tab and nothing else changes - which is the point of writing the boundary down now rather than discovering it when the tab is built.

**5. The JavaScript half: we adopt all of it, page included, and keep only the pane around it.** `options.js`, `render.js` (the camera policy, and the hardest part to get right), `apply.js`, `notify.js`, `states.js` and `logo.js` all take a viewer instance and plain data - `buildDisplayOptions(config, overrides, geometry)` takes the geometry as an argument rather than measuring a window - so every one of them is usable unchanged in a pane.

**`createPage` is adopted too, and the core grows four injections so that it can be.** It cannot be used as it stands - measured, four bindings assume the host was handed a whole browsing context: `getSize()` reads `window.innerWidth`/`innerHeight` (`page.js:137`), the container is `document.getElementById("cad_viewer")` (`:177`), the only way in is `window.addEventListener("message")` (`:246`), and it resizes on the window's own resize event (`:226`). A fifth is a number rather than a binding: `normalizeHeight` is `height - 65`, which this pane measured as wrong - it over-reserves and shows as a bottom margin visibly larger than the left and top ones, which is why `reservedHeight()` takes the canvas's own top offset instead.

**The decision is his, and the reason is feature parity rather than tidiness: all hosts must have the same set of features, for support and maintenance.** `createPage`'s message dispatch *is* the feature set - `screenshot`, `set_relative_time`, `backend_response`, `ui` through `applyConfig`, `animation`, `logo`, and `data` with `currentStates`/`restoreStates`. A shell of our own would get exactly the features we re-implement and would drift on every one of them; screenshot is the live example, six lines there and absent here. So the core takes `container`, a `getSize`, an exported `handleMessage(data)` and an explicit `resize()`, with the window listeners still attached by default - which leaves both page hosts behaving exactly as they do today - and the measured canvas offset becomes the shared height, falling back to the constant only before the canvas is in the DOM. That change lands in `ocp-viewer-core` and is pushed into all three adopted hosts, because each keeps its own copy of the JavaScript and a stale one silently tests the old code.

So `src/viewer/viewer.js` becomes the host shell around `createPage`: it keeps the pane geometry it measures (the width and height normalisation and the geometry probe, which exist because guessing at the chrome produced both a clipped status line and a dead bottom margin), the binary path through `rehydrate`, and the IPC wiring that turns a frame into a `handleMessage` call. It loses `SNAKE_TO_CAMEL`, `KEY_OVERRIDES`, `RENDER_KEYS`, `VIEWER_KEYS`, `displayOptions`, `mapOptions`, `notify`, `showModel` and the `reset_camera` branch inside it - every one of which is a copy of something the core owns.

**Two files shrink to nothing or nearly.** `src/viewer/logo.js` is byte-identical to the core's copy - same sha256, 200,733 bytes, two hosts shipping one file - and goes. `resolveInstances` in `src/viewer/rehydrate.js` is a 23-line copy written because three-cad-viewer fused instance resolution into its base64 decode; **5.0.3 exports `decodeBuffers` and `resolveInstances` separately**, so the copy goes with the version bump. **`rehydrate` itself stays**, and so does `kernel/build123d_studio/buffers.py`: the core still calls `numpy_to_buffer_json` on the show path (`show.py:818`), so the arrays still arrive base64'd and something still has to undo it. "arrays as arrays" was in the old plan and was not built.

**6. Dependencies and versions.** `three-cad-viewer` 5.0.0 to 5.0.3, which is the core's peer floor (`>=5.0.3 <5.1.0`). In the runtime, `ocp_vscode==4.0.1` **goes out** and `ocp-viewer-core` comes in, bringing `ocp-tessellate>=3.5.0,<3.6.0`, `orjson` and `threejs-materials` with it. **Nothing in the chain is published to PyPI or npm yet** - the core is at 0.1.0 - but every project is now on GitHub, so **the Python half is consumed as an editable install** and the JavaScript half as a `yarn pack` tarball until it is. That is the mechanism the environment already has: `[tool.uv.sources]` with `editable = true`, which is live with no further command, and which the Settings dialog already offers for build123d. Two traps come with it: an editable install records the version at install time, so anything *resolving* against it sees the old number after a bump until it is re-installed; and a `file:` dependency in `package.json` is a development state, never a committed one.

**7. The kernel package stops compensating for ocp_vscode.** `kernel/build123d_studio/__init__.py` is 242 lines, and most of what is not `show` is there because it imports another host: `_require_ocp_vscode` and the guard around the private `_convert`; `set_port(OCP_PORT)` to stop viewer discovery running and probing this kernel's TCP iopub port; and two `warnings.filterwarnings` suppressing complaints from a viewer we deliberately refuse to talk to. All of it goes - the core is a library, and there is no other viewer in the picture. What is left is `StudioComms`, the settings source and the bindings, so `from build123d_studio import show` sits beside `from ocp_vscode import show` and `from ocp_viewer import show`. **Per-host imports, and the reason is a receipt in this repository**: with a VS Code viewer on the default 3939, our `show()` once silently drew into *that* viewer. A single universal import with the host discovered at runtime makes that representable; four host packages make it impossible to write down.

#### What we gain, and the one thing we give back

Never had, and arriving with the adoption: the **camera policy**, so a second `show()` stops throwing the view away; **tree-state preservation** across a re-show; `set_defaults`/`get_defaults`/`reset_defaults` and `set_viewer_config`; `push_object`, `show_objects`, `show_clear`, `remove_object` and `save_screenshot`; `status`; value validation, so `theme="drak"` raises instead of vanishing into the renderer; and the eleven `studio_*` setters. And the **measurable splash**, parked in this section since 2026-08-06: `showLogo()` renders `src/viewer/logo.js` in the frontend alone and our `MeasurementBackend` skips `super().__init__`, so no mapping ever reached it - the core's `backend.py` loads `logo.py` at start, so this follows from adoption rather than being built.

**What we give back is `_can_draw`.** The core's `show_all` inherits ocp_vscode's permissive list test; ours is recursive, and it exists because a plain list of floats produced a model with a header and no geometry, which three-cad-viewer does not come back from - the application needed a force-quit. It is the one place the golden master is deliberately not preserved, and it moves up rather than staying here. **Already given back**: the delta-not-snapshot notification, which is `EVENT_KEYS` in the core now.

#### How it is verified

The narrowest suite the change reaches, and then a real build. `yarn test:sidecar` and `yarn test:integration` cover the model socket, so the mapping split lands with both; `yarn test:frontend`'s viewer fixture is a real tessellation captured through `_convert` and `split_buffers`, so it is recaptured through the core rather than hand-edited. The camera policy, tree-state preservation and the measurable splash are all things only he can see, so they go to him as a numbered test on a dev build.

#### Decided

**A `StudioComms` for the show path and nothing for the backend.** **Per-host imports**, `from build123d_studio import show`. **`workspace_config()` is the Settings dialog's viewer parameters** - today `theme` alone, later a Viewer tab with Dark mode moved into it. **`status` is answered from a picture the sidecar holds**, never by asking the browser synchronously; ours can be fresh, and nothing shared may depend on that. **Event keys are never accumulated.** **base64 stays on the show path**, so `split_buffers` and `rehydrate` stay with it.

**`exclude_keys` is `("cad_width", "height", "viewer", "anchor", "pinning", "port")`** - his ruling. **`createPage` is adopted**, and the core grows the four injections that let a pane host use it. **The Python half is consumed as an editable install** from the GitHub checkouts until the chain is published. **Screenshot has to come.**

**And the rule above all of them, his, for support and maintenance: every host has the same set of features, not only the same defaults.** A default that differs from the other three viewers is a defect; so is a feature one host has and another does not. That is what decides a question like `createPage` - the shared page is where the feature set lives, so the answer is to make it fit the fourth host rather than to write a fourth answer.

#### Open

Nothing blocking. Two things to settle while building: where a screenshot is written to and how the user asks for one, and whether the measured canvas offset replaces the constant `65` for the two page hosts as well - it should, but that is a change to their layout and wants seeing on a real build.

#### Not in scope

Rewriting three-cad-viewer. Changing the VS Code wire format. More than one viewer per process, though the core's instance state stops making it impossible.
...
### Nothing outlives a quit - the case the measurements above missed

**The sidecar itself outlived its application, and it happened seven times on one afternoon.** Found on 2026-08-15 by counting: seven sidecar trees with `ppid 1`, aged fifty minutes to two and a quarter hours, each still holding a kernel, a console and a measurement backend. Everything above this heading is about what the *sidecar's children* survive, and all of it still holds - the children are fine, because each has a contract that takes it with us. What nothing asserted is that the sidecar reaches the end of its own shutdown.

**Two mechanisms, both of them ours.** _One_, a quit asks twice by design: `stopSidecar` sends the `shutdown` frame and then closes stdin, so a webview that dies before the frame lands is still heard. Both were running the whole teardown, concurrently, and they met in the middle - each waiting on something the other had already taken. _Two_, and this is what made losing that race unrecoverable: **websockets creates its connection thread non-daemon** (`sync/server.py:285`, 16.1.1), which is the thread the `shutdown` frame is handled on. So when the stdin path finished first and `main()` returned, CPython's own teardown joined a thread that was itself stuck inside the same shutdown, and the process sat there for ever having logged nothing. From the outside: an application that had quit, and a sidecar that had not.

**Measured, not inferred, and by elimination rather than by a debugger**: no process held the write end of the orphan's stdin pipe (so EOF had arrived), its main thread was parked on a lock rather than in a read (so it was inside the teardown), basedpyright was gone in all seven while the console, kernel and measurement process were alive (so it had passed the language server and had not finished the measurement backend). That last one names the step: `MeasurementService.stop()` took the service lock, a request holds that lock for its whole round trip, and the write inside it goes into a pipe of 64 kB carrying a mapping of megabytes. Reproduced in a test with a backend that reads nothing: un-applied, both the stop and the request hang for ever.

**What changed.** `Sidecar.stop()` has **one owner** - the second caller waits for the first and finds it done, rather than running the chain again - and a **deadline**: `SHUTDOWN_DEADLINE`, eight seconds, after which the process says which step it was on and exits anyway. Both paths now end in `os._exit`, so no interpreter teardown ever joins somebody else's thread again. A step that raises no longer strands the ones after it. `MeasurementService.stop()` kills rather than asking politely and takes no lock, because asking politely means closing stdin and closing a buffered writer flushes it; its watchdog is armed **before** the write rather than after, so the half of a round trip that can actually block is the half it covers. Six clean quits measured end to end afterwards, 1.6 to 2.3 s each, nothing left behind. `tests/integration.py` now closes a sidecar's stdin and asserts the tree is gone, which is the half of the contract only a real quit exercises - the check above it kills the sidecar, which nothing can survive and which therefore says nothing about being asked.

### The three that turn a silent failure into a sentence

Written the same day, out of an afternoon in which every fault was silent - and the standing question behind all three, his: *"what would happen from a user experience if the run starts happily, but something in the background is not set up properly?"* The answer was "nothing".

**And what the measurement backend says about itself, which is where this was first tested and first found wanting.** He killed `measure_process` by hand to see the chip and nothing happened, correctly: the service notices on the next request, respawns, re-sends the mapping and answers, so measurement kept working and no part of the application ever said a process of ours had been killed. Silent recovery is the same defect as silent failure one step further in. It now reports `degraded` on a respawn and `ready` when the replacement actually answers, deduplicated so an ordinary measurement announces nothing - otherwise a frame per click while somebody drags a camera. **Then his next question, measured rather than answered from the design: what if it cannot be started at all?** Two shapes that behaved differently. A spawn that fails - no interpreter, no script - was reported at once. One that spawns and dies on its import, which is what a half-finished package upgrade looks like, announced `degraded` for the respawn and then **nothing**, because only the broken round trip knows and `_exchange` swallowed it: the chip sat on amber for ever while every measurement produced nothing. A broken round trip now reports `failed` with the reason, once, and not while the process is being stopped on purpose - killing it on the way out has exactly the same shape from the inside. **And then the chip read `failed` beside a measurement that plainly worked**, which was one more of the same family: the branch that made the splash measurable - `if self._mapping is None: return self._exchange(message)` - returned straight out of the request, past the line that announces recovery. So with a model shown the state came back to `ready` and with nothing shown the identical working measurement left `failed` on screen. The branch is gone rather than patched: nothing to send simply falls through, and `_request` has one exit, so a measurement means the same thing whatever is on screen.

**What deliberately reports nothing: the console.** His ruling. A console that dies writes `[console exited]` into the pane the user is already looking at, and Restart Kernel replaces it - so it is neither silent nor without a way out, and a chip would be a second way of saying something the screen has already said. The test for whether a subsystem belongs in the health model is not "can it break" but **"can it break without the user being told, and with nothing they can do about it"**.

**A subsystem chip in the toolbar.** The kernel indicator speaks for the kernel alone, and the Backend tab covers a bad *start* - the application opens on it and only switches to the console when nothing is unwell. What was missing is what dies afterwards: the model channel, the measurement backend, the language server, the console. The chip carries the worst state of `src/health.js`, a tooltip naming every subsystem, and clicking it opens the Backend tab. It is hidden while all is well, because a toolbar that always carries a status light teaches people not to look at it. **A new `failed` also brings that tab forward by itself; `degraded` does not** - a failure means something has stopped and the reason is already written there, so showing it is not an interruption, and there is no dialog to dismiss. Edge-triggered on the subsystem name: level-triggered, every later report would drag the pane back off the console for a failure already read.

**Nothing blocks silently, on the kernel's side of the socket.** `transport.py` bounded only the reply. Connect and send are bounded now, and the errors name the hop - "the viewer is not answering" rather than a bare `NotConnected`, because a bare one reads as "there is no application", which is the one thing it is not. It matters more since the adoption: a `show()` makes two blocking reads before it draws anything, so an unbounded write means a stalled sidecar stops Python rather than delaying a frame. And `modelsock` no longer reads and delivers on its accept loop: a connection is read on a thread of its own and a **COMMAND is answered there**, while models, mappings and configs go to one worker in arrival order - a mapping that overtook its own model would describe the wrong one. The delivery queue holds two, deliberately: filling it blocks the connection's thread, which stops it draining its socket, which blocks the kernel's `sendall` - the backpressure the serial loop gave for free, kept.

**A stall becomes evidence.** On 2026-08-15 an execute hung for two minutes and the log's entire record was `Kernel warm-up started: importing build123d` with no matching finish; the diagnosis came from timestamps and a `sample` of the process, neither of which a user can be asked for. `sidecar/stall.py` watches each kernel request by msg_id and, at five seconds and then every thirty, writes one line saying what everything else was doing: whether the kernel process is alive, whether we believe it busy, how many connections the model channel is reading and how many messages are queued behind it, whether the measurement backend is alive and mid-request, and the depth of every lane. Nothing is cancelled and nothing times out - a Run may legitimately take minutes - the log simply stops going quiet. **The rule it encodes: a line recording something starting is half a sentence, and the other half not arriving is itself worth writing down.**
