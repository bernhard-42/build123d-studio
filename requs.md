...
**Which also let the upgrade lose its name list.** It used to pass `--upgrade-package` per package, on the grounds that a blanket `--upgrade` would move ocp_vscode and break the viewer. The pins were already the guarantee, so the list was restating them. A blanket upgrade means one sentence a user can hold - *everything moves as far as its own constraint allows* - and it removes the only reason to parse names out of the user's text and put them on a command line. The cost is transitive drift, which is the nature of the button.
