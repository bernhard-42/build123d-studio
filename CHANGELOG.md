...
- **three-cad-viewer 5.0.7: the viewer releases a model's GPU memory when the next one is shown.** With 5.0.6 a large STEP assembly (219 shapes, a 45 MB mesh) left about 6 GB behind on every show, so a handful of them took the window past WebKit's limit, which then killed and reloaded the page. Measured on the same files with 5.0.7: the same sequence stays under 550 MB.
- **When the window's page is reloaded, the previous kernel is stopped and a dead viewer says so.** WebKit ends a page that grows past its memory limit — a handful of large STEP models will do it, until the viewer's memory is fixed in a later release — and reloads it. The reloaded page used to start a second sidecar and kernel while the first ones ran on, a gigabyte each; it now stops what the previous page left before starting its own. And on such a page WebGL is often unavailable, which used to mean every show "rendered" into an empty pane in silence: the toolbar chip now reads **failed** with "The viewer cannot draw: WebGL is not available in this window. Restart build123d Studio."
- **The snippets are a file you can edit.** `snippets.json` beside the settings is written from the shipped set — Jern's build123d CodeCAD speedmodeling snippets — the first time Studio starts on a machine, and from then on that file is the whole set: read at every start and when Settings is applied, so an edit, an addition or a removal takes effect at the next start. Delete the file to get the shipped set back. It used to be read only on top of a built-in set, so the shipped snippets were nowhere to be seen or changed.
- **Clicking a CAD file in the file tree shows it.** An STL, STEP (`.step`/`.stp`), BREP, DXF or SVG used to open as text — or be refused as "not a text file". A click now runs build123d's importer for it on the kernel and shows the result, and the line that did it is in the console, after an `# Importing frame.step ...` that stays on screen while a large STEP loads: `from build123d import import_step; from build123d_studio import show, Camera; _imported = import_step("…/frame.step"); show(_imported, reset_camera=Camera.RESET)`. The result is bound to `_imported` only, so clicking through a folder of models does not fill the namespace; keep one with `frame = _imported`. The variable explorer shows `_imported` — the one underscore name it does. And the kernel indicator stays on busy while the import runs: the kernel's own "busy" for a request can arrive seconds late when an importer holds the GIL, so a run of yours that is next in line now counts as running from the moment the one before it finishes.
