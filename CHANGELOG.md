...
- **Show from the variable explorer.** Right-click a variable and choose *Show* to run `show(<name>)` on the kernel. Rows below a variable have no name of their own, so *Show* is greyed out there.
...
- **Startup and diagnostics.** When the environment cannot be prepared, the splash now shows the underlying error, including the shell's or curl's own message, and the path of the log file. The log begins with the machine's details (OS, memory, disk space, relevant environment variables — credentials redacted) and records the exit code of every command. A first start no longer fails on Windows machines whose command prompt is broken by a stale AutoRun registry entry; the splash explains the entry instead.
